...
- **Inline Completion** — tab-автокомплит (полупрозрачные подсказки при вводе)
...
1. Установите тот же `.vsix` в Cursor и задайте API-ключ (боковая панель или
   `ExCashAI: Установить API-ключ`)
2. В боковой панели ExCashAI: **Применить в Cursor** (ключ + слияние списка моделей
   в `state.vscdb`)
3. Полностью закройте и перезапустите Cursor (Reload Window недостаточно)
4. В Agent / списке моделей выберите `ex-…` (ExCashAI)
5. Чтобы снова пользоваться Composer/Pro Cursor: в боковой панели нажмите
   **Модели Cursor** (BYOK остаётся настроенным; «Применить» снова включает
   ExCashAI)
6. По желанию: **Скрыть модели Cursor** / **Показать модели Cursor** в боковой
   панели переключают штатные записи Cursor в списке моделей (если редактор
   это поддерживает)
...
- В Cursor пишутся `ex-<modelId>` и для thinking — `ex-<modelId>(effort)`,
  где `effort` — один из уровней, задаваемых в боковой панели (расширение записывает
...
| `ExCashAI: Переключить inline-автодополнение`    | Вкл/выкл tab-автокомплит        | оба     |
| `ExCashAI: Выбрать модель inline-автодополнения` | Модель автокомплита             | оба     |
...
Действия только в боковой панели Cursor (не через палитру команд):
**Модели Cursor**, **Скрыть модели Cursor**, **Показать модели Cursor**.
...
- Статистика токенов — по моделям и число запросов
