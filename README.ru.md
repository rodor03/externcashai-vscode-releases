...
1. Палитра команд / Command Palette (`Ctrl+Shift+P` / `Cmd+Shift+P` на macOS)
...
5. Чтобы снова пользоваться Composer/Pro Cursor: нажмите **Модели Cursor**
   (**Use Cursor models**) в сайдбаре (BYOK остаётся настроенным; «Применить»
   снова включает ExternCash)
...
- В Cursor пишутся `ex-<modelId>` и для thinking — `ex-<modelId>(effort)`,
  где `effort` — один из уровней, задаваемых в сайдбаре (расширение записывает
  только допустимые значения).
- `openAIBaseUrl` должен содержать путь `/v1` (расширение перезаписывает URL
  без `/v1`).
- Успешное «Применить» ≠ «Agent уже работает» — нужен полный перезапуск Cursor
  и проверка на модели `ex-…`.

### Запасной вариант: буфер обмена

Если нет CLI `cursor`, «Применить» копирует ключ в буфер — вставьте вручную в
...
- Не логируйте и не шарьте сырой API-ключ; «Копировать» / «Применить»
  используют ключ только внутри процесса расширения.
- После удаления расширения в настройках Cursor могут остаться записи `ex-*`.
  ...
  снова нажмите «Применить» после чистой установки.
...
| `ExCashAI: Синхронизировать модели с Cursor`     | Синхронизация после «Применить»  | Cursor  |
...
Действия только в сайдбаре Cursor (не через палитру команд): **Модели Cursor**
...
- Секция **Cursor** (только в Cursor) — Применить / Копировать / Синхронизировать /
  Модели Cursor (Use Cursor models) / Скрыть·Показать модели Cursor
- Статистика токенов
...
- API-ключ ExCashAI
- Для «Применить» / «Синхронизировать» в Cursor: `sqlite3` (обязательно) и
  CLI `cursor` (если доступен)
